<template>
  <div class="h-screen">
    <div class="w-full">
      <div class="flex flex-col w-full text-gray-700 bg-gray-200 dark-mode:text-gray-200 dark-mode:bg-gray-800 flex-shrink-0">
        <div class="flex-shrink-0 px-8 py-4 flex flex-row items-center justify-between">
          <NuxtLink to="/" class="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg">
            Stock
          </NuxtLink>
          <button class="rounded-lg rounded-lg focus:outline-none focus:shadow-outline" @click="$data.isNavOpened = !$data.isNavOpened">
        ...
        <nav :class="{'block': $data.isNavOpened, 'hidden': !$data.isNavOpened}" class="flex-grow px-4 pb-4 bg-gray-100">
          <div
